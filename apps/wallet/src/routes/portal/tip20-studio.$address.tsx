...
export const Route = createFileRoute('/portal/tip20-studio/$address')({
  component: Tip20StudioDashboard,
});
...
function Tip20StudioDashboard(): ReactElement {
  const { address: tokenAddress } = Route.useParams();
  const navigate = useNavigate();
  const { address: userAddress } = useTempo();

  const { stablecoin, isLoading, isNotFound, changeTransferPolicy, removeStablecoin, refresh } =
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[12px] text-muted-foreground font-mono">{tokenAddress}</span>
        </div>
      </div>
