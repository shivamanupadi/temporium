...
import { ArrowLeft, Plus, Minus, Check, ExternalLink, DollarSign, Loader2, Droplets, AlertCircle } from 'lucide-react'
...
import { getPoolInfo, getLiquidityBalance, getExplorerTxUrl, type PoolInfo } from '@/lib/tempo-client'
...
function LiquidityPage(): ReactElement | null {
  ...
  const hasBalance = actionType === 'add'
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-[0_0_0_1px_rgba(0,0,0,0.03),0_1px_2px_-1px_rgba(0,0,0,0.03)] p-5 space-y-4">
        {/* Action Tabs */}
        <Tabs value={actionType} onValueChange={(v) => { setActionType(v as ActionType); setAmount('') }}>
                ...
                <SelectTrigger className="w-full h-10 bg-white shadow-sm border-gray-200">
                  {userToken && (
                    <div className="flex items-center gap-1.5">
                      ...
                      >
                        <DollarSign className="h-3 w-3" style={{ color: getTokenColors(userToken.symbol).text }} />
                      </div>
                ...
                <SelectTrigger className="w-full h-10 bg-white shadow-sm border-gray-200">
                  {validatorToken && (
                    <div className="flex items-center gap-1.5">
                      ...
                      >
                        <DollarSign className="h-3 w-3" style={{ color: getTokenColors(validatorToken.symbol).text }} />
                      </div>
            ...
            {isLoadingPool ? (
              ...
              <>
                ...
                <div className="flex justify-between text-[12px]">
                  ...
                  <span className="text-foreground">
                    {formatAmount(poolInfo.reserveValidatorToken.toString(), validatorToken.decimals)}
                  </span>
                </div>
                <div className="flex justify-between text-[12px]">
            ...
            <button
              onClick={() => {
                if (actionType === 'add' && validatorBalance.data?.value && validatorToken) {
                  setAmount(formatAmount(validatorBalance.data.value.toString(), validatorToken.decimals))
        ...
        >
          {isLoadingPool ? 'Loading pool...' : `Review ${actionType === 'add' ? 'Add' : 'Remove'} Liquidity`}
        </Button>
      </div>
      ...
      <Dialog open={modalState !== null} onOpenChange={handleCloseModal}>
        <DialogContent hideClose={modalState === 'pending'} className="sm:max-w-sm p-0 overflow-hidden">
          {modalState === 'confirm' && userToken && validatorToken && (
            <div className="p-5">
              <DialogTitle className="sr-only">Confirm {actionType === 'add' ? 'Add' : 'Remove'} Liquidity</DialogTitle>
              <DialogDescription className="sr-only">Confirm your liquidity action</DialogDescription>
              ...
              <div className="text-center mb-5">
                ...
                <p className="text-2xl font-semibold text-foreground">
                  {amount} <span className="text-muted-foreground font-normal">
                    {actionType === 'add' ? validatorToken.symbol : 'LP'}
                  </span>
                </p>
              ...
              <div className="bg-muted/50 rounded-lg p-3 space-y-2.5 mb-5">
                <div className="flex justify-between text-[13px]">
                  ...
                  <span className="text-foreground">{userToken.symbol}/{validatorToken.symbol}</span>
                </div>
                <div className="flex justify-between text-[13px]">
          ...
          {modalState === 'pending' && userToken && validatorToken && (
            <div className="p-8 text-center">
              ...
              <p className="text-[14px] text-muted-foreground">
                {actionType === 'add' ? 'Adding' : 'Removing'} {amount} {actionType === 'add' ? validatorToken.symbol : 'LP'}
              </p>
            </div>
          )}
          ...
          {modalState === 'success' && txHash && userToken && validatorToken && (
            <div className="p-5 text-center">
              ...
              <p className="text-[13px] text-muted-foreground mb-4">
                {amount} {actionType === 'add' ? validatorToken.symbol : 'LP'} in {userToken.symbol}/{validatorToken.symbol} pool
              </p>
