...
export async function saveContact(
  ...
  const response = await apiPost<ContactApiResponse>('/v1/contacts', {
    name: contact.name,
...
export async function updateContact(
  ...
  const response = await apiPatch<ContactApiResponse>(`/v1/contacts/${id}`, {
    name: updates.name,
  });
  return apiToContact(response);
}
...
export async function deleteContact(id: string, _owner: Address): Promise<void> {
  await apiDelete(`/v1/contacts/${id}`);
}

export async function getContactById(id: string): Promise<Contact | null> {
  try {
    const response = await apiGet<ContactApiResponse>(`/v1/contacts/${id}`);
    return apiToContact(response);
  } catch {
    return null;
  }
...
export async function getContactsByOwner(_owner: Address): Promise<Contact[]> {
  const response = await apiGet<ContactApiResponse[]>('/v1/contacts');
