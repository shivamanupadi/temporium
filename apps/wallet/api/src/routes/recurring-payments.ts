...
/**
 * GET /v1/recurring-payments/config
 * Public — returns the fee config so the frontend can include the fee
 * transfer in the creation batch transaction.
 */
recurringPaymentsRouter.get('/config', async c => {
  const network = c.get('networkConfig').network;
  const raw = c.env.PLATFORM_FEE_RECURRING_AMOUNT ?? '0';
...
recurringPaymentsRouter.get(
  '/',
  jwtAuth,
  zValidator('query', recurringPaymentQuerySchema),
  async c => {
 ...
 */
recurringPaymentsRouter.get('/:id', jwtAuth, zValidator('param', idParamSchema), async c => {
  const db = createDb(c.env.DB);
  const owner = getWalletAddress(c);
 ...
 */
recurringPaymentsRouter.delete('/:id', jwtAuth, zValidator('param', idParamSchema), async c => {
  const db = createDb(c.env.DB);
  const owner = getWalletAddress(c);
  const { id } = c.req.valid('param');
