...
/**
 * Check if a bigint value represents "unlimited" (max uint128)
 * Max uint128 is commonly used in smart contracts to represent unlimited supply
 */
const MAX_UINT128 = 2n ** 128n - 1n;

export function isUnlimitedSupply(value: bigint | undefined): boolean {
  if (!value) return false;
  // Check if value is max uint128 (or very close to it)
