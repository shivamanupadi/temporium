...
interface UseStablecoinReturn {
  ...
  isLoading: boolean
  isNotFound: boolean
  mintTokens: (params: { to: Address; amount: bigint }) => Promise<{ receipt: { transactionHash: string } }>
  burnTokens: (params: { amount: bigint }) => Promise<{ receipt: { transactionHash: string } }>
  pauseToken: () => Promise<{ receipt: { transactionHash: string } }>
  unpauseToken: () => Promise<{ receipt: { transactionHash: string } }>
  setSupplyCap: (supplyCap: bigint) => Promise<{ receipt: { transactionHash: string } }>
  grantRoles: (params: { to: Address; roles: readonly TokenRole[] }) => Promise<{ receipt: { transactionHash: string } }>
  revokeRoles: (params: { from: Address; roles: readonly TokenRole[] }) => Promise<{ receipt: { transactionHash: string } }>
  burnBlocked: (params: { from: Address; amount: bigint }) => Promise<{ receipt: { transactionHash: string } }>
...
export function useStablecoin(tokenAddress: string): UseStablecoinReturn {
  ...
  const pauseToken = useCallback(async () => {
    return pauseTokenBase(normalizedAddress)
  }, [pauseTokenBase, normalizedAddress])

  const unpauseToken = useCallback(async () => {
    return unpauseTokenBase(normalizedAddress)
